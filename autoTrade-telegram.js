...
bot.on("channel_post", async (ctx) => {
  try {
    ...
    if (!text.startsWith("/approve_wallet")) return;
    ...
    if (!user) {
      return ctx.telegram.sendMessage(
        chat.id,
        "❌ Wallet has not requested this channel."
      );
    }
    ...
    console.error("approve_wallet channel_post error:", err);
  }
});
