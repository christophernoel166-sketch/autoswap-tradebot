...
const telegramUserSchema = new mongoose.Schema({
  ...
  sessionKey: String, // (legacy) consider removing or hashing
  sessionExpire: Date,
});

// ✅ IMPORTANT: avoid colliding with your main models/User.js
export const TelegramUser =
  mongoose.models.TelegramUser || mongoose.model("TelegramUser", telegramUserSchema);
...
export async function openSession(tgId, password) {
  const u = await TelegramUser.findOne({ tgId });
  if (!u || !u.encryptedWallet) throw new Error("Wallet not set.");

  const key = deriveKey(password, u.salt);
